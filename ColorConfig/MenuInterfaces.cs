...
namespace ColorConfig.MenuUI
{
        ...
        public class ExpeditionColorInterface : PositionedMenuObject, IDoPerPage
        {
            ...
            public void PopulatePage(int offset)
            {
                ...
                this.ApplyPerPageList(bodyNames, (name, num) =>
                {
                    ...
                    menu.MutualMenuObjectBind(buttons.ValueOrDefault((num - 1) % PerPage), bodyButton, true);
                    buttons.Add(bodyButton);
                    borders.Add(bodyColorBorder);
                    bodyColors.Add(bodyColor, num);
                });
                menu.MutualMenuObjectBind(prevNextButton?.prevButton, buttons.FirstOrDefault(), true);
                menu.MutualMenuObjectBind(buttons.LastOrDefault(), prevNextButton?.nextButton, true);
                bodyButtons = [.. buttons];
            ...
            public void ActivateButtons()
            {
                ...
                menu.MutualMenuObjectBind(prevNextButton.prevButton, bodyButtons.FirstOrDefault(), true);
                menu.MutualMenuObjectBind(bodyButtons.LastOrDefault(), prevNextButton.nextButton, true);
            }
            public void DeactivateButtons() => this.ClearMenuObject(ref prevNextButton);
    ...
    public class HexTypeBoxPages : PositionedMenuObject, IDoPerPage
    {
        ...
        public void PopulatePage(int offset)
        {
            ...
            menu.MutualMenuObjectBind(prevNextButton?.prevButton, hexBoxes?.Keys?.FirstOrDefault()?.elementWrapper, true);
            menu.MutualMenuObjectBind(hexBoxes?.Keys?.LastOrDefault()?.elementWrapper, prevNextButton?.nextButton, true);
        }
    ...
    public class HexTypeBox : PositionedMenuObject, ICopyPasteConfig
    {
        ...
        public virtual bool ShouldCopyPaste => hexTyper?.MouseOver == true && !menu.FreezeMenuFunctions;
        ...
        public virtual void SaveNewHSL(Vector3 hsl) => currentHSL = hsl;
        public virtual void SaveNewRGB(Color rgb) => currentHSL = Custom.RGB2HSL(rgb);
        ...
        {
            if (!shouldSaveNewTypedColor || saveNewTypedColor == null) return;
    ...
    public class SliderPages : PositionedMenuObject, ICanTurnPages, IGetOwnInput, ICopyPasteConfig
    {
        ...
        public bool ShouldCopyPaste => ModOptions.Instance.CopyPasteForSliders.Value && !menu.FreezeMenuFunctions && CopyPasteSlider != null;
        ...
        public virtual void TryGetInput(Player.InputPackage input, Player.InputPackage lastInput)
        {
            if (!menu.manager.menuesMouseMode && PagesOn && (prevButton?.Selected == true || nextButton?.Selected == true || sliderOOOs.Any(x => x.Selected)))
            {
                ...
                if (input.mp && !lastInput.mp)
                    PrevPage();
                else if (input.pckp && !lastInput.pckp)
                    NextPage();
            }
